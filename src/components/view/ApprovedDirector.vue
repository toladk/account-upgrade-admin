                     ...
                     <div class="iq-card">
                                    ...
                                    <form>
                                        <div class="row mt-4 mb-3">
                                            ...
                                            <div class="col-md-4 pl-0">
                                                <button class="button w-100 btn btn-primary" @click.prevent="getApprovedFiltered()" style="margin-top: 34px;background-color: #229C57; border-color: #229C57; padding: 10px 10px 10px 10px;"><i class="ri-download-fill"></i>Get Approved Director</button>
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="iq-card-header d-flex justify-content-between">
                           <div class="iq-header-title">
                              <h4 class="card-title">Approved Director</h4>
                           </div>
                        ...
                        <div class="iq-card-body">
                           <el-table :data="approvedDirectorSearchList.slice((currentPage-1)*PageSize,currentPage*PageSize)" style="width: 100%" v-loading="loadingData">
                            ...
                            <div class="col-lg-12 mt-4">
                                <div class="row">
                                    <div class="col-md-12 pl-0 pr-0" style="display: flex">
                                        <div class="col-lg-6 pl-0">Total Director: {{ approvedDirectorSearchList.length }}</div>
                                        <div class="col-lg-6 pr-0">
                                            <div class="" style="float: right">
                                                <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
                                                    :current-page="currentPage" :page-size="PageSize"
                                                    background layout="prev, pager, next" :total="approvedDirectorSearchList.length"></el-pagination>
                                            </div>
                                        </div>
                                    </diV>
                                </div>
...
import VuePdfEmbed from 'vue-pdf-embed'
    ...
    export default {
        ...
        mounted() {
            this.getApprovedDirector();
        },

        methods: {
            async getApprovedDirector(){
                try{
                    this.loadingData = true;
                    const response = await axios.get(`${config.baseUrl}/directors/approved`, {headers: {
                        Authorization: `Bearer ${sessionStorage.getItem('token')}`
                    }})
                    this.approvedDirectorList = response.data.respData;
                    this.approvedDirectorSearchList = this.approvedDirectorList;
                    ElNotification({ title: 'Approved Director', message: 'Approved Director Fetch Successfully !!', type: 'success' })
                    this.loadingData = false;
                    this.formatDate();
                } catch (error){
                    this.loadingData = false;
                    ElNotification({ title: 'Approved Director', message: error.response.data.message, type: 'error' })
                }
            },
            async getApprovedFiltered(){
                try{
                    this.loadingData = true;
                    const response = await axios.get(`${config.baseUrl}/directors/approved?DateFrom=${this.approveFormFilter.dateApprovedFrom}&DateTo=${this.approveFormFilter.dateApprovedTo}`, {headers: {
                        Authorization: `Bearer ${sessionStorage.getItem('token')}`
                    }})
                    this.approvedDirectorList = response.data.respData;
                    this.approvedDirectorSearchList = this.approvedDirectorList;
            ...
            SearchDocument() {
                ...
                this.approvedDirectorSearchList = this.approvedDirectorList.filter(res => {
