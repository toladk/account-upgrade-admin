    ...
    <div>
                     ...
                     <div class="iq-card">
                        <div class="iq-card-header d-flex justify-content-between">
                           <div class="iq-header-title">
                              <h4 class="card-title">Pending Signatories</h4>
                           </div>
                        ...
                        <div class="iq-card-body">
                           <el-table :data="pendingSignatorySearchList.slice((currentPage-1)*PageSize,currentPage*PageSize)" style="width: 100%" v-loading="loadingData">
                               <el-table-column label="#" width="50px">
                                ...
                                <template #default="scope">
                                    <div class="mt-3 d-flex">
                                        ...
                                        <el-tooltip class="box-item" effect="dark" content="Approve / Reject Signatory" placement="top-start"><button type="button" class="btn btn-outline-info mb-3" @click="apprSignatory(scope.row.id)"><i class="ri-check-line"></i></button></el-tooltip>
                                    </div>
                                </template>
                                </el-table-column>
                            </el-table>
                            ...
                            <div class="col-lg-12 mt-4">
                                <div class="row">
                                    <div class="col-md-12 pl-0 pr-0" style="display: flex">
                                        <div class="col-lg-6 pl-0">Total Signatory: {{ pendingSignatorySearchList.length }}</div>
                                        <div class="col-lg-6 pr-0">
                                            <div class="" style="float: right">
                                                <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
                                                    :current-page="currentPage" :page-size="PageSize"
                                                    background layout="prev, pager, next" :total="pendingSignatorySearchList.length"></el-pagination>
                                            </div>
                                        </div>
                                    </diV>
                                </div>
        ...
        <!-- FOR APPROVE DIRECTOR -->
         <el-drawer v-model="drawerApprove" :direction="direction" size="40%">
            <template #title>
            <h4>Approve / Reject Signatory</h4>
            </template>
            <template #default>
            <div>
                <div class="row">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="d-inline-block w-100 mt-2">
                                                <button type="submit" class="btn w-100 btn-primary mt-4" style="border-color: #229C57; padding-top: 11px; padding-bottom: 12px; font-size: 13px; background-color: #229C57" :disabled="approveForm.comment === ''" @click.prevent="approveSignatory()">Approve Signatory</button>
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="d-inline-block w-100 mt-2">
                                                <button type="submit" class="btn w-100 btn-danger mt-4" style="border-color: orangered; padding-top: 11px; padding-bottom: 12px; font-size: 13px; background-color: orangered" :disabled="approveForm.comment === ''" @click.prevent="rejectSignatory()">Reject Signatory</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                pendingSignatoryList: [],
                pendingSignatorySearchList: [],
        ...
        mounted() {
            this.getPendingSignatory();
        },

        methods: {
            async getPendingSignatory(){
                try{
                    this.loadingData = true;
                    const response = await axios.get(`${config.baseUrl}/signatories/pending`, {headers: {
                        Authorization: `Bearer ${sessionStorage.getItem('token')}`
                    }})
                    this.pendingSignatoryList = response.data.respData;
                    this.pendingSignatorySearchList = this.pendingSignatoryList;
                    ElNotification({ title: 'Pending Signatory', message: 'Pending Signatory Fetch Successfully !!', type: 'success' })
                    this.loadingData = false;
                    this.formatDate()
                } catch (error){
                    this.loadingData = false;
                    ElNotification({ title: 'Pending Signatory', message: error.response.data.message, type: 'error' })
                }
            },
            ...
            async approveSignatory(){
                try {
                    ...
                    const response = await axios.post(`${config.baseUrl}/signatory/approve`, payload, {headers: {
                        Authorization: `Bearer ${sessionStorage.getItem('token')}`
                    }})
                    this.approveDetails = response.data.respData;
                    this.loadingApprove = false;
                    this.drawerApprove = false;
                    ElNotification({ title: 'Approve Signatory', message: 'Signatory Approved Successfully !!', type: 'success' })
                    this.getPendingSignatory();
                    this.approveForm.comment = '';
                } catch(error){
                    this.loadingApprove = false;
                    this.approveForm.comment = '';
                    ElNotification({ title: 'Approve Signatory', message: error.response.data.message, type: 'error' })
                }
            },
            async rejectSignatory(){
                try {
                    ...
                    this.loadingApprove = false;
                    this.drawerApprove = false;
                    ElNotification({ title: 'Reject Signatory', message: 'Signatory Rejected Successfully !!', type: 'success' })
                    this.getPendingSignatory();
                    this.approveForm.comment = '';
                } catch(error){
                    this.loadingApprove = false;
                    this.approveForm.comment = '';
                    ElNotification({ title: 'Reject Signatory', message: error.response.data.message, type: 'error' })
                }
            },
            ...
            SearchDocument() {
                ...
                this.pendingSignatorySearchList = this.pendingSignatoryList.filter(res => {
