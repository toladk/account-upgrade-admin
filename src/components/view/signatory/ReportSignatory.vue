                  ...
                  <div class="col-lg-12">
                     <div class="iq-card">
                                            ...
                                            <div class="col-md-3">
                                                <div class="form-group">
                                                    <label for="email">Account Number</label>
                                                        <input type="number" v-model="accountForm.AccountNumber" class="form-control" placeholder="Enter Account No">
                                                </div>
                                            </div>
                                            <div class="col-md-3">
                                                <div class="form-group">
                        ...
                        <div class="iq-card-body">
                           <el-table :data="reportSignatorySearchList.slice((currentPage-1)*PageSize,currentPage*PageSize)" style="width: 100%" v-loading="loadingData">
                            ...
                            <div class="col-lg-12 mt-4">
                                <div class="row">
                                    <div class="col-md-12 pl-0 pr-0" style="display: flex">
                                        <div class="col-lg-6 pl-0">Total Signatory: {{ reportSignatorySearchList.length }}</div>
                                        <div class="col-lg-6 pr-0">
                                            <div class="" style="float: right">
                                                <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
                                                    :current-page="currentPage" :page-size="PageSize"
                                                    background layout="prev, pager, next" :total="reportSignatorySearchList.length"></el-pagination>
                                            </div>
                                        </div>
                                    </diV>
                                </div>
...
import dayjs from 'dayjs'
    ...
    export default {
        ...
        validations(){
            return {
                accountForm: {
                    AccountNumber: { required: helpers.withMessage('Account is required', required), numeric: helpers.withMessage('accept numbers only', numeric) },
        ...
        methods: {
            async getReportDocument(){
                try{
                    ...
                    if (this.accountForm.DateCreatedFrom === '' && this.accountForm.DateCreatedTo === ''){
                        ...
                        const response = await axios.get(`${config.baseUrl}/signatories/report?AccountNumber=${this.accountForm.AccountNumber}&DateCreatedFrom=${this.accountForm.DateCreatedFrom}&DateCreatedTo=${this.accountForm.DateCreatedTo}&Approver=${this.accountForm.Approver}&Status=${this.accountForm.Status}`, {headers: {
                            Authorization: `Bearer ${sessionStorage.getItem('token')}`
                        }})
                        this.reportSignatoryList = response.data.respData;
                        this.reportSignatorySearchList = this.reportSignatoryList;
            ...
            SearchDocument() {
                ...
                this.reportSignatorySearchList = this.reportSignatoryList.filter(res => {
            ...
            downloadReport(){
                let filename = 'Signatory Report.csv';
                let text = Papa.unparse(this.reportSignatorySearchList);
